...
# 数据集准备
test_data = torchvision.datasets.CIFAR10(r"Dataset/data_Cifar10", train=False,
...
def main(WeightsPath, epoch):
    ...
    # TODO 疑问1：检测准确率会随着detect()的循环次数增加而减少。已解决，错误原因：原先的text_input.append处于detect中，导致循环添加text信息
    ...
    while epoch > 0:
        ...
        with torch.no_grad():
            ...
            for data in test_dataloader:
                ...
                # #第一次尝试：让imgs从{Tensor:(1,3,32,32)}变成{Tensor:(1,3,224,224)}
                # K = torch.ones(1,3,7,7)#转置卷积核
                # tconv.weight.data = K  # 确定转置卷积核
                # tconv = nn.ConvTranspose2d(3, 3, kernel_size=7, stride=7, bias=False)
                # imgs = tconv(imgs)
                """
                转置卷积失败报错
...
if __name__ == '__main__':
    while True:
        ...
        # 主程序
        accuracy = main(weightspath, epoch)
