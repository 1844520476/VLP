...
class opt:
    ...
    def hyp(self):
        print(f't为最小透射率，一般取0.25。投射图t 的值过小——>图像会整体向白场过度')
        self.threshold_value = float(input(f't:'))
        print(f'w为去雾程度，一般取0.95。w的值越大，去雾效果越明显')
        self.dehaze_degree = float(input(f'w:'))
...
def quwu(img):
    ...
    if img is None:
        ...
        # 3.待处理图片所在目录地址
        imgInput = r'Input/emotion'
        # 4.处理后图片保存地址
        imgFile = r'Output/haze_test_310'
    ...
    if __name__ == '__main__':  # 这里必须是__main__（在本模块执行的时候__name__会保存为__main__），不能是__DeHaze__或__dehaze__(函数/模块名)
        ...
        img_ed = DeHaze(Opt)
        print('[2]去雾算法正在被调用')
    img_Ed = r'{}/{}'.format(imgFile, img_ed)
    return img_Ed+'.jpg'

if __name__ == '__main__':
    img_trans = None
    ImgEd = quwu(img_trans)
    print('去雾后的图片地址和名称:{}'.format(ImgEd))
